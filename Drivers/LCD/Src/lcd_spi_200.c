...
uint16_t  LCD_Buff[1024*16];        // LCD缓冲区，16位宽（每个像素点占2字节）
...
{
   LCD_DC_Data;     // 数据指令选择 引脚输出高电平，代表本次传输数据
...
void LCD_WriteData_16bit_Bulk(uint16_t* data, uint32_t count)
{
    ...
    uint8_t spi_buffer[320 * 2]; // 静态缓冲区避免重复分配
    ...
    for(uint32_t i = 0; i < count; i++) {
        spi_buffer[i*2] = (data[i] >> 8) & 0xFF;   // 高字节在前
        spi_buffer[i*2+1] = data[i] & 0xFF;       // 低字节在后
    }
	//printf("666\n");
...
//    
//    spi_transfer_complete = 0;
//    HAL_SPI_Transmit_DMA(&hspi6, (uint8_t*)data, count * 2); // 16位数据需乘2
//    
//    // 等待传输完成（或通过回调处理）
//    while (!spi_transfer_complete);
//}
...
//嗯，用户之前问过如何实现LVGL的disp_flush函数，用的是STM32H723ZGT6和Keil。现在他补充说用的是ST7789的TFT屏幕，分辨率320x240，SPI接口。我需要帮他写出这个函数的最佳实现。

//首先，用户可能已经完成了基本的硬件初始化，比如SPI和GPIO的配置，但需要具体的刷新函数。考虑到SPI的速度可能成为瓶颈，应该尽量优化数据传输，避免逐像素写入，而是使用DMA或硬件加速。

//ST7789支持区域写入，所以应该先设置写入的窗口（x和y的范围），然后一次性发送整个区域的数据。这样可以减少命令发送的次数，提高效率。用户可能不知道这一点，需要指导他使用设置窗口命令。

//另外，颜色格式需要确认。LVGL默认使用LV_COLOR_16_SWAP，可能是RGB565，但可能需要调整字节顺序。要确保发送的颜色数据与屏幕期待的格式一致，可能需要交换高低字节。

//SPI的传输方式，如果使用阻塞传输，可能会比较慢，尤其是在高分辨率下。建议使用DMA传输，这样在数据传输时CPU可以处理其他任务，提升性能。但用户如果资源紧张，也可以先实现阻塞传输，之后再优化到DMA。

//还需要处理LVGL的缓冲区。如果用户使用双缓冲区，可能需要在传输完成后通知LVGL准备下一帧。但在这个函数中，主要任务是刷新指定的区域，所以每次刷新后必须调用lv_disp_flush_ready来通知LVGL。

//用户可能没有正确配置SPI的速率，需要提醒他确保SPI时钟足够高，比如使用STM32H7的高速SPI，可能达到几十MHz。同时，GPIO的片选和DC引脚需要正确控制，比如在发送命令和数据时切换DC引脚的状态。

//另外，考虑内存对齐和缓冲区的大小。如果颜色数据是16位的，每个像素占两个字节，发送时需要确保数据指针正确，并且传输的字节数是区域大小的两倍。

//可能还需要处理屏幕的旋转，设置正确的MADCTL命令，确保显示方向正确。但用户可能已经在初始化时配置好了，这里可以暂时不提，除非用户遇到显示方向问题。

//总结步骤：设置窗口 -> 发送像素数据 -> 通知LVGL完成。使用SPI的DMA传输来优化性能，同时处理好颜色格式和字节顺序。另外，提供阻塞传输的示例，供用户根据实际情况选择。

//以下是针对 STM32H723 + ST7789 (320x240 SPI) 优化实现的 disp_flush 函数，采用DMA传输和屏幕窗口机制。我这里提供一个完整实现模板，需要你先完成硬件层驱动：
...
//#define TFT_SPI_HANDLE hspi1  // 你的SPI句柄

//extern SPI_HandleTypeDef TFT_SPI_HANDLE;

//// 发送命令/数据的快捷方式
//static void tft_send_cmd(uint8_t cmd) {
//    HAL_GPIO_WritePin(TFT_DC_PORT, TFT_DC_PIN, GPIO_PIN_RESET); // DC=0:命令模式
//    HAL_GPIO_WritePin(TFT_CS_PORT, TFT_CS_PIN, GPIO_PIN_RESET); // CS拉低
//    HAL_SPI_Transmit(&TFT_SPI_HANDLE, &cmd, 1, HAL_MAX_DELAY);
//    HAL_GPIO_WritePin(TFT_CS_PORT, TFT_CS_PIN, GPIO_PIN_SET); // CS拉高
//}

//static void tft_send_data(uint8_t *data, uint16_t len) {
//    HAL_GPIO_WritePin(TFT_DC_PORT, TFT_DC_PIN, GPIO_PIN_SET);   // DC=1:数据模式
//    HAL_GPIO_WritePin(TFT_CS_PORT, TFT_CS_PIN, GPIO_PIN_RESET); // CS拉低
...
//    tft_send_cmd(0x2C); // 写入内存命令

//    // 启用DMA传输像素数据（建议使用32位传输）
...
//    // 注意：需要等待DMA传输完成后再调用lv_disp_flush_ready()
//    // 可以通过DMA完成中断或在主循环中轮询实现
//}

//// DMA传输完成回调函数（在stm32h7xx_it.c中实现）
...
//确保SPI时钟 ≥ 20MHz（ST7789最高支持62.5MHz）
//配置DMA通道（推荐使用32位传输）
//正确连接DC、CS引脚
//LVGL配置（lv_conf.h）：
...
//#define LV_COLOR_16_SWAP 1  // ST7789需要交换字节
//初始化代码：
...
//启用STM32H7的SPI硬件CRC（如果需要）
//使用32位宽度的DMA传输
//性能提升技巧：

//将SPI时钟设置为最大允许值
//使用Memory-mapped模式（如果支持）
//启用STM32H7的Cache
//如果DMA资源紧张，这里提供一个阻塞版本的基础实现：
...
//确保SPI时序配置正确（CPOL/CPHA）
//屏幕初始化参数可能需要微调
//根据屏幕实际分辨率调整参数
//如果出现颜色异常，检查LV_COLOR_16_SWAP设置
//STM32H7的SPI需要正确配置Cache策略



//好的，用户之前已经询问了如何为STM32H723ZGT6和ST7789屏幕实现LVGL的disp_flush函数，并且我提供了一个使用DMA的版本。现在用户请求一个不使用DMA的函数实现。我需要理解用户为什么需要非DMA版本，可能是DMA资源被其他外设占用，或者项目复杂度较低，不需要DMA的高效传输。

//首先，用户明确提到使用ST7789的SPI接口，分辨率为320x240。不使用DMA意味着需要采用阻塞式传输，即通过轮询方式发送数据。这种情况下，SPI传输会占用CPU资源，直到所有数据发送完成，可能会影响系统实时性，但对于小型项目或资源有限的情况是可行的。

//接下来，我需要确保非DMA版本的函数结构正确。原DMA版本中，设置显示窗口、发送命令和数据的部分仍然适用，只需修改数据传输部分，用HAL_SPI_Transmit替代HAL_SPI_Transmit_DMA。同时，由于没有DMA，传输完成后不需要等待中断，可以直接调用lv_disp_flush_ready。

//需要注意的是，阻塞式传输可能会导致刷新速率较慢，尤其是全屏刷新时，数据量大，SPI传输时间较长。因此，建议用户使用双缓冲机制，并合理设置LVGL的刷新区域，减少单次传输的数据量。

//另外，用户之前可能已经配置了SPI的CPOL和CPHA，需要确认这些参数是否正确，否则数据传输会出错。同时，颜色格式（RGB565，字节交换）也需要与LVGL配置一致，避免颜色显示异常。

//用户可能没有提及，但需要考虑SPI时钟速度。STM32H723的SPI时钟可以设置较高，以提升传输速度，但需在ST7789支持的范围内（最高约62.5MHz）。提高SPI速度可以减少阻塞时间，改善性能。

//最后，提供代码时需要详细注释，说明每一步的作用，特别是设置窗口和发送数据的部分，帮助用户理解流程。同时提醒用户初始化ST7789时正确设置颜色模式和显示方向，确保显示正常。

//以下是针对 ST7789 的 不使用DMA 的阻塞式 disp_flush 函数实现，适用于 STM32H723 + SPI 驱动：
...
//通过0x2A/0x2B命令一次性设置刷新区域，减少SPI命令交互次数
//使用4字节数据包设置X/Y范围（ST7789要求的大端格式）
//数据传输优化：

//一次性发送整个区域数据（而非逐像素发送）
//保持CS引脚在整个数据传输期间为低电平
//使用HAL_SPI_Transmit的阻塞式传输确保数据完整性
//颜色格式适配：

//c
//复制代码
//// 在 lv_conf.h 中配置
//#define LV_COLOR_DEPTH 16      // ST7789使用RGB565
//#define LV_COLOR_16_SWAP 1     // 需要交换字节序
//性能提升建议：
//提高SPI时钟：

//在CubeMX中设置SPI时钟到最高允许值（ST7789最高支持62.5MHz）
//确保SPI模式为 CPOL=1, CPHA=1（模式3）
//使用双缓冲：

//c
//复制代码
//// 在显示驱动初始化时添加
//#define BUF_SIZE 320 * 40  // 根据可用RAM调整
//static lv_color_t buf1[BUF_SIZE];
//static lv_color_t buf2[BUF_SIZE];
...
//复制代码
//// 在屏幕初始化代码中确保以下设置：
//tft_send_cmd(0x36);       // MADCTL（屏幕方向）
//tft_send_data(0x00, 1);   // 根据需要调整方向参数
//tft_send_cmd(0x3A);       // 颜色格式
//tft_send_data(0x55, 1);   // 0x55=16bit/pixel
//典型问题排查：
//花屏/错位：

//检查SPI的CPOL/CPHA设置
//确认LV_COLOR_16_SWAP是否与屏幕需求一致
//验证屏幕初始化序列是否正确
//刷新速度慢：
...
//复制代码
//// 在main.c的主循环中添加优先级设置
//HAL_SPI_SetClockPrescaler(&hspi1, SPI_BAUDRATEPRESCALER_2); // 提高SPI时钟
//局部刷新异常：

//确保area->x1/x2/y1/y2的数值在0-319和0-239范围内
//检查窗口设置命令的数据打包是否正确
//如果需要进一步优化，可以采用以下增强策略：

//c
//复制代码
//// 在disp_flush函数开始时添加：
//__HAL_SPI_ENABLE(&TFT_SPI_HANDLE); // 确保SPI已使能

//// 传输完成后添加（针对H7系列）：
//SCB_CleanInvalidateDCache(); // 如果使用Cache需要清理缓存
